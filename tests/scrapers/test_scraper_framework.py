...
from services.data.database.connection import init_database, close_database, db_manager
...
class ScraperTestFramework(ABC):
    ...
    def __init__(self, scraper_name: str, scraper_function: Callable, expected_source_name: str):
    ...
    async def _test_source_exists(self):
        ...
        try:
            ...
            source = await db_manager.execute_sql_one(query, f"%{self.expected_source_name}%")
            ...
            if 'source_verification' not in self.test_results:
                self.test_results['source_verification'] = {'status': 'error', 'error': str(e)}
            raise
    ...
    async def _test_scraper_execution(self):
        ...
        try:
            ...
            pre_count_query = "SELECT COUNT(*) as count FROM articles WHERE source_id = $1"
            pre_count = await db_manager.execute_sql_one(pre_count_query, self.source_id)
            
            # Ejecutar scraper
            article_ids = await self.scraper_function()
            
            # Contar artículos después
            post_count = await db_manager.execute_sql_one(pre_count_query, self.source_id)
            ...
            self.test_results['scraper_execution'] = {'status': 'error', 'error': str(e)}
        ...
        try:
            ...
            if total > 0:
                quality_metrics = {
                    ...
                    'word_count_completeness': (quality['with_word_count'] / total) * 100,
                    'language_completeness': (quality['with_language'] / total) * 100,
                    'country_completeness': (quality['with_country'] / total) * 100,
                    'url_uniqueness': (quality['unique_urls'] / total) * 100 if total > 0 else 100
                }
            ...
            if total > 0:
                print(f"   📝 Title completeness: {quality_metrics['title_completeness']:.1f}%")
                print(f"   🔗 URL uniqueness: {quality_metrics['url_uniqueness']:.1f}%")
                print(f"   📅 Date completeness: {quality_metrics['date_completeness']:.1f}%")
                print(f"   🔑 Metadata completeness: {quality_metrics['hash_completeness']:.1f}%")
    ...
    async def _test_performance(self):
        ...
        try:
            if 'scraper_execution' in self.test_results and self.test_results['scraper_execution']['status'] == 'success':
                ...
                performance_metrics = {
                    'articles_per_second': articles_added / execution_time if execution_time > 0 else 0,
                    'seconds_per_article': execution_time / articles_added if articles_added > 0 else 0,
                    'execution_efficiency': 'excellent' if execution_time < 60 else 'good' if execution_time < 120 else 'needs_optimization'
                }
                ...
                print(f"   ⚡ Articles per second: {performance_metrics['articles_per_second']:.2f}")
                print(f"   ⏱️  Seconds per article: {performance_metrics['seconds_per_article']:.2f}")
...
class BreastCancerOrgTest(ScraperTestFramework):
    ...
    async def test_scraper_specific(self):
        ...
        try:
            ...
            print(f"   ✅ URL pattern validation: {url_check['count']} articles match expected pattern")
            
        except Exception as e:
            self.test_results['specific_breast_cancer_org'] = {'status': 'error', 'error': str(e)}
