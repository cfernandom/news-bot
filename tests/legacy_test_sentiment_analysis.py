...
def test_sentiment_analyzer():
    ...
    for i, case in enumerate(test_cases, 1):
        ...
        print(
            f"Result: {result['sentiment_label']} (confidence: {result['confidence']:.3f})"
        )
        print(
            f"Scores: pos={result['scores']['positive']:.3f}, neg={result['scores']['negative']:.3f}, neu={result['scores']['neutral']:.3f}"
        )
...
def test_nlp_integration():
    ...
    if result.sentiment_data:
        ...
        print(
            f"Sentiment: {sentiment['sentiment_label']} (confidence: {sentiment['confidence']:.3f})"
        )
...
async def test_database_integration():
    ...
    try:
        ...
        for article_data in articles_data:
            ...
            print(
                f"   Source: {article_data[5] or 'Unknown'}"
            )  # source_name from database
            print(
                f"   Sentiment: {sentiment['sentiment_label']} ({sentiment['confidence']:.3f})"
            )
