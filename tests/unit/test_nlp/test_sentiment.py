...
class TestSentimentAnalyzer:
    ...
    def test_analyze_sentiment_positive_text(self):
        ...
        assert result["sentiment_label"] in [
            "positive",
            "neutral",
        ]  # Medical context is more conservative
    ...
    def test_analyze_sentiment_with_title(self):
        ...
        assert (
            result["processed_text_length"] > result["text_length"]
    ...
    def test_medical_sentiment_thresholds(self):
        ...
        for case in test_cases:
            scores = {"pos": 0.3, "neg": 0.1, "neu": 0.6, "compound": case["compound"]}
            label, confidence = analyzer._interpret_medical_sentiment(scores)
            assert (
                label == case["expected"]
            ), f"Failed for compound score {case['compound']}"
