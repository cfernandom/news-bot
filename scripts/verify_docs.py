...
class DocVerifier:
    ...
    def extract_links(self, content: str) -> List[Tuple[str, str, str]]:
        ...
        return [(title.strip(), path.strip(), desc.strip() if desc else "") 
            ...
            if link_matches:
                ...
                for title, path in link_matches:
                    if not path.startswith('#') and not path.startswith('http'):
                        ...
                        updated_line = re.sub(r'\s*-\s*(?:✅|\(Pendiente\))\s*', ' - ', updated_line)
                        
                        # Add new status indicator
                        if ' - ' in updated_line:
                            updated_line = updated_line.replace(' - ', f' - {status} ', 1)
                        else:
                            # If no description, add status
                            updated_line = updated_line.replace(f']({path})', f']({path}) - {status}')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Verify documentation links and update status')
