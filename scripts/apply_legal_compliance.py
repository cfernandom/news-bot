...
class LegalComplianceMigrator:
    ...
    async def apply_database_migration(self):
        ...
        migration_file = project_root / "services/data/database/migrations/002_legal_compliance.sql"
        ...
        try:
            ...
            if self.dry_run:
                ...
                logger.info(f"Migration SQL preview (first 200 chars): {migration_sql[:200]}...")
                return True
        ...
        try:
            ...
            for article in articles:
                ...
                if self.stats['articles_processed'] % 10 == 0:
                    logger.info(f"Processed {self.stats['articles_processed']}/{len(articles)} articles")
    ...
    async def _process_article_compliance(self, article):
        ...
        try:
            ...
            if self.dry_run:
                logger.debug(f"DRY RUN: Would update article {article_id} with compliance data")
                return
            ...
            async with self.db_manager.get_connection() as conn:
                await conn.execute(
                    update_query,
                    robots_compliant, copyright_status, content_type,
                    legal_review_status, retention_expires, article_id
                )
    ...
    async def _process_source_compliance(self, source):
        ...
        try:
            ...
            if self.dry_run:
                logger.debug(f"DRY RUN: Would update source {source_id} with compliance data")
                return
            ...
            async with self.db_manager.get_connection() as conn:
                await conn.execute(
                    update_query, robots_url, crawl_delay, source_id
                )
    ...
    async def create_compliance_audit_entry(self):
        ...
        try:
            ...
            async with self.db_manager.get_connection() as conn:
                await conn.execute(
                    audit_query,
                    'system', 0, 'compliance_migration', 'completed',
                    details, 'migration_script'
                )
