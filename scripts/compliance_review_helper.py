...
class ComplianceReviewHelper:
    ...
    async def get_compliance_summary(self):
        ...
        for row in status_data:
            ...
            print(f"  {row['legal_review_status']}: {row['count']} artículos ({percentage:.1f}%) - {avg_length:,} chars promedio")
        
        print(f"\n📈 Total de artículos: {total_articles}")
        
        print("\n🌐 Fuentes de Noticias:")
        for row in sources_data:
            scraping_status = "✅ Permitido" if row['scraping_allowed'] else "❌ Bloqueado" if row['scraping_allowed'] is False else "❓ Desconocido"
            print(f"  {row['name']}: {row['article_count']} artículos - {scraping_status} - Delay: {row['crawl_delay_seconds']}s")
    ...
    async def remove_full_content_keep_summaries(self, limit: int = None):
        ...
        print(f"  Longitud promedio contenido: {int(stats['avg_content_length'] or 0):,} caracteres")
        print(f"  Longitud promedio resumen: {int(stats['avg_summary_length'] or 0):,} caracteres")
    ...
    async def _log_compliance_action(self, action: str, details: str):
        ...
        async with self.db_manager.get_connection() as conn:
            await conn.execute(
                audit_query,
                'articles', 0, action, 'completed', details_json, 'compliance_helper'
            )

async def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Compliance Review Helper for PreventIA")
    parser.add_argument('--action', choices=['preview', 'approve-all', 'remove-content', 'summary'], 
