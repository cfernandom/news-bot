...
from services.nlp.src.topic_classifier import get_topic_classifier, MedicalTopic
...
async def process_topic_classification(batch_size: int = 50):
    ...
    try:
        ...
        for offset in range(0, total_articles, batch_size):
            logger.info(f"📦 Processing batch {offset//batch_size + 1} (articles {offset+1}-{min(offset+batch_size, total_articles)})")
...
async def show_topic_statistics(db_manager):
    ...
    logger.info("   " + "="*70)
    logger.info(f"   {'Topic':<15} {'Count':<8} {'Avg Confidence':<15} {'Percentage':<10}")
    logger.info("   " + "-"*70)
    
    for row in results:
        topic, count, avg_conf, percentage = row
        logger.info(f"   {topic:<15} {count:<8} {avg_conf or 0:<15} {percentage or 0:<10}%")
