...
class DocQualityChecker:
    ...
    def _check_single_file(self, file_path: Path, relative_path: str):
        ...
        try:
            ...
            self._add_issue(relative_path, 0, 'file_error', 'error', 
                          f"Could not read file: {e}")

    def _check_metadata_presence(self, file_path: str, content: str):
        """Check if important documents have metadata blocks."""
        important_files = [
            'README.md', 'architecture/system-overview.md', 
            'api/services/nlp-api.md', 'development/standards/'
        ]
        ...
        if is_important and not has_metadata:
            self._add_issue(file_path, 1, 'missing_metadata', 'warning',
    ...
    def _check_language_consistency(self, file_path: str, content: str):
        ...
        spanish_indicators = len(re.findall(r'\b(el|la|los|las|un|una|de|del|que|para|con|por)\b', content.lower()))
        english_indicators = len(re.findall(r'\b(the|a|an|of|to|for|with|by|in|on|at)\b', content.lower()))
        
        is_spanish = spanish_indicators > english_indicators
        
        # Check expected language based on path
        expected_english = any(pattern in file_path for pattern in self.language_rules['technical_docs'])
        expected_spanish = any(pattern in file_path for pattern in self.language_rules['team_docs'])
        
        if expected_english and is_spanish:
            self._add_issue(file_path, 1, 'language_inconsistency', 'warning',
            ...
            self._add_issue(file_path, 1, 'language_inconsistency', 'info',
    ...
    def _check_required_sections(self, file_path: str, content: str):
        ...
        if missing_sections:
            self._add_issue(file_path, 1, 'missing_sections', 'info',
    ...
    def _check_link_format(self, file_path: str, lines: List[str]):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if malformed_links:
                self._add_issue(file_path, i, 'malformed_link', 'warning',
            ...
            if relative_paths:
                self._add_issue(file_path, i, 'relative_path', 'info',
            ...
            if line.strip().startswith('```'):
                if not in_code_block:
                    ...
                    if not lang and 'example' in line.lower():
                        self._add_issue(file_path, i, 'missing_language', 'info',
                ...
                if re.search(r'rm\s+-rf\s+/', line) or 'sudo rm' in line:
                    self._add_issue(file_path, i, 'unsafe_command', 'warning',
    ...
    def _check_heading_structure(self, file_path: str, lines: List[str]):
        ...
        for line_num, level, title in headings:
            if level > prev_level + 1:
                self._add_issue(file_path, line_num, 'heading_hierarchy', 'info',
        ...
        if duplicates:
            self._add_issue(file_path, 1, 'duplicate_headings', 'info',
    ...
    def _check_line_length(self, file_path: str, lines: List[str]):
        ...
        if len(long_lines) > 5:  # Only report if it's a pattern
            self._add_issue(file_path, long_lines[0], 'long_lines', 'info',
    ...
    def _check_security_patterns(self, file_path: str, content: str):
        ...
        for pattern, issue_type in suspicious_patterns:
            ...
            for match in matches:
                ...
                self._add_issue(file_path, line_num, issue_type, 'warning',
    ...
    def generate_report(self, issues: List[QualityIssue]) -> str:
        ...
        report.extend([
            "📈 Issue Types Summary:",
            *[f"  {itype}: {count}" for itype, count in sorted(issue_types.items())],
            ""
        ])
