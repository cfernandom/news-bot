"""
Breast Cancer Now Scraper - PostgreSQL Storage
...
async def get_breast_cancer_now_source_id() -> Optional[int]:
    query = "SELECT id FROM news_sources WHERE base_url LIKE '%breastcancernow.org%' LIMIT 1"
    result = await db_manager.execute_sql_one(query)
    return result["id"] if result else None


async def scrape_breast_cancer_now_to_postgres() -> List[int]:
    URL = "https://breastcancernow.org/about-us/news-personal-stories"
    BASE_URL = "https://breastcancernow.org"

    print(f"🔍 Starting scrape of {URL}")

    source_id = await get_breast_cancer_now_source_id()
    if not source_id:
        print("❌ ERROR: Breast Cancer Now source not found in news_sources table")
        return []
    ...
    async with async_playwright() as p:
        try:
            ...
            article_links = await page.evaluate(
                ...
                () => {
                    const links = [];
                    const elements = document.querySelectorAll('.news-item a, .story-link a, .content-item a, h2 a, h3 a');
                    for (let el of elements) {
                        if (el.href && el.textContent && el.href.includes('/news/') || el.href.includes('/stories/')) {
                            links.push({
                                url: el.href,
            ...
            for i, article_info in enumerate(
                ...
                try:
                    await asyncio.sleep(
                        3
                    )  # Compliance: Rate limiting (charity respect)
                    ...
                    article_data = await page.evaluate(
                        ...
                        () => {
                            // Extract title
                            let title = '';
                            const titleSelectors = ['h1', '.page-title', '.article-title', '.news-title'];
    ...
    print(
        f"🎉 Breast Cancer Now scraping completed! Added {len(articles_added)} articles"
    )
...
async def main():
    ...
    try:
        articles = await scrape_breast_cancer_now_to_postgres()
        print(f"\n📊 SUMMARY:")
        print(f"Articles scraped: {len(articles)}")
        print(f"Source: Breast Cancer Now")
        print(f"Database: PostgreSQL")
        print(
            f"Compliance: ✅ Robots.txt respected, rate limited, charity guidelines followed"
        )
