...
async def get_news_medical_source_id() -> Optional[int]:
    query = "SELECT id FROM news_sources WHERE base_url LIKE '%news-medical.net%' LIMIT 1"
    ...
    try:
        async with async_playwright() as p:
            ...
            try:
                ...
                print(f"✅ Successfully fetched HTML with Playwright ({len(html)} chars)")
    ...
    for article in soup.find_all("div", class_="row"):
        ...
        try:
            ...
            result = await db_manager.execute_sql_one(
                insert_query,
                source_id, title, full_url, "", summary, date, current_time,
                'en', 'International', 'pending', content_hash, word_count
            )
...
if __name__ == "__main__":
    async def main():
        from services.data.database.connection import init_database, close_database
        try:
            await init_database()
