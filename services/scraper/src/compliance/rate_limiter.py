...
class RateLimiter:
    ...
    async def wait_if_needed(self, url: str) -> float:
        ...
        if self._request_counts[domain] % 10 == 0:
            logger.info(f"Domain {domain}: {self._request_counts[domain]} requests processed")
    ...
    def get_stats(self) -> Dict[str, Dict]:
        ...
        """
        stats = {}
        for domain in set(list(self._last_request_time.keys()) + list(self._request_counts.keys())):
            stats[domain] = {
