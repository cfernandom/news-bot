...
from services.scraper.src.extractors.www_newsnetwork_mayoclinic_org import scraper__mayoclinic
...
async def scrape_articles() -> list[Article]:
    ...
    article4 = await scraper__mayoclinic()
    article5 = await scraper__sciencedaily()
    article6 = await scraper__news_medical()
    articles.extend(article1 + article2 + article3 + article4 + article5 + article6)
    return articles