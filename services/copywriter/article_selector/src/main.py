...
def load_prompt_template() -> str:
    path = os.path.join(os.path.dirname(__file__), "prompts", "choose_articles_prompt.txt")
...
def choose_articles_for_structure(
    ...
    prompt = prompt_base.replace("{structure_json}", structure_json).replace("{articles_json}", articles_json)

    print("[SELECTOR] → Escogiendo artículos para estructura:")
    print("STRUCTURE_JSON:", structure_json[:200] + ("..." if len(structure_json) > 200 else ""))
    print("CANDIDATES_JSON:", articles_json[:200] + ("..." if len(articles_json) > 200 else ""))
    ...
    response = client.chat.completions.create(
        model="gpt-4.1-nano",
        messages=[
            {"role": "system", "content": "Eres un asistente para asignar artículos a secciones de una newsletter médica."},
    ...
    try:
        ...
        print(text)
        raise RuntimeError(f"Error validando respuesta de choose_articles_for_structure: {e}\nRespuesta LLM: {text}")