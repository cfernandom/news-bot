...
def generate_summary_structured(
    ...
    prompt = (
        prompt_base
        .replace("{section_title}", section)
        .replace("{full_text}", safe_text)
    )
    ...
    print("Texto a resumir (primeros 200 caracteres):", full_text[:200].replace("\n", " ") + "...")
    ...
    try:
        ...
        print(f"[SUMMARY][ERROR] JSON inválido o esquema incorrecto para sección \"{section}\": {e}")
        print("LLM devolvió:")
        print(text)
        return None
