...
def load_prompt_template() -> str:
    path = os.path.join(os.path.dirname(__file__), 'prompts', 'propose_structure_prompt.txt')
...
def propose_structure(articles: List[Dict]) -> StructureTemplate:
    ...
    response = client.chat.completions.create(
        model="gpt-4.1-nano",
        messages=[
            {"role": "system", "content": "Eres un asistente para diseñar estructuras de newsletter."},
    ...
    try:
        ...
        print(text)
        raise RuntimeError(f"Error validando respuesta de propose_structure: {e}\nRespuesta LLM: {text}")